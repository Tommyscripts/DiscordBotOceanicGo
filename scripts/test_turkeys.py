...
# Test turkeys balances
print('\n--- Testing turkeys balances ---')
TEST_USER_ID = 888888888888

print('Adding 10 turkeys to user', TEST_USER_ID)
cur.execute("INSERT INTO turkeys_balances(user_id, turkeys) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET turkeys = turkeys + ?", (TEST_USER_ID, 10, 10))
conn.commit()
row = cur.execute("SELECT turkeys FROM turkeys_balances WHERE user_id = ?", (TEST_USER_ID,)).fetchone()
print('Balance after +10:', row[0] if row else 0)

print('Setting turkeys to 42 for user', TEST_USER_ID)
cur.execute("INSERT INTO turkeys_balances(user_id, turkeys) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET turkeys = ?", (TEST_USER_ID, 42, 42))
conn.commit()
row = cur.execute("SELECT turkeys FROM turkeys_balances WHERE user_id = ?", (TEST_USER_ID,)).fetchone()
print('Balance after set 42:', row[0] if row else 0)

print('Subtracting 5 turkeys')
cur.execute("INSERT INTO turkeys_balances(user_id, turkeys) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET turkeys = turkeys + ?", (TEST_USER_ID, -5, -5))
conn.commit()
row = cur.execute("SELECT turkeys FROM turkeys_balances WHERE user_id = ?", (TEST_USER_ID,)).fetchone()
